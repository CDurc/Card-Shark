...
@onready var Gen_anime: AnimationPlayer = $"SPM - Red/AnimationPlayer"
...
func idle_task(goal_node: Node3D) -> void:
	var task_time: float = float(current_goal_task["time"])
	var cycles: int = int(floor(task_time / 4.5))
	for i in range(cycles):
		Gen_anime.play("Idle")
		await Gen_anime.animation_finished

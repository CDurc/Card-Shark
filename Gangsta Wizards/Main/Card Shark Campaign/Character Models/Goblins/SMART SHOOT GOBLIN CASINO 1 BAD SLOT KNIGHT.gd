...
@onready var bullet_path = preload("res://Particles/bullet_1m.tscn")
...
@onready var anime1 = $casino_badslot/Upper
@onready var anime2 = $casino_badslot/Lower
...
func _physics_process(delta: float) -> void:
	...
	if knockback_t > 0.0:
		...
		if (target.global_transform.origin - nav_agent.target_position).length() > 0.15:
			...
			anime1.play("Walk")
			#anime2.play("Walking Upper") #COME BACK TO
...
func damage(amount):
	...
	if healthbar_control.can_appear:
		#healthbar_control.visible = true #TODO RETURN HERE
